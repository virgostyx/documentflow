...
RSpec.describe Documents::CheckInOrganizer do
  ...
  describe ".call" do
    context "when checked in by the user who checked it out" do
      it "creates a new document file version" do
        expect {
          described_class.call(document: document, current_user: visa_actor, file: new_file, comment: "Fixed typo")
      ...
      it "replaces the document's main_file with the new content" do
        described_class.call(document: document, current_user: visa_actor, file: new_file, comment: nil)
    ...
    context "when checked in by someone other than the user who checked it out" do
      ...
      it "fails and does not create a version" do
        expect {
          described_class.call(document: document, current_user: other_user, file: new_file, comment: nil)
