...
RSpec.describe EmailTemplatePolicy, type: :policy do
  subject(:policy) { described_class.new(user, email_template) }

  let(:entity) { create(:entity) }
  let(:department) { create(:department, entity: entity) }
  let(:email_template) { create(:email_template, entity: entity, created_by: creator) }
  ...
  describe "#show?" do
    ...
    context "when the template is restricted to a department" do
      let(:email_template) { create(:email_template, entity: entity, created_by: creator, department: department) }
  ...
  describe "Scope" do
    let!(:entity_wide_template) { create(:email_template, entity: entity, created_by: creator, name: "Entity wide") }
    let!(:department_template) do
      create(:email_template, entity: entity, created_by: creator, department: department, name: "Dept scoped")
    end
    let!(:other_entity_template) { create(:email_template, entity: create(:entity), created_by: creator, name: "Other entity") }

    it "gives owners every template in their entity" do
      result = EmailTemplatePolicy::Scope.new(owner, EmailTemplate).resolve
    ...
    it "gives members entity-wide templates plus templates from their own departments" do
      ...
      result = EmailTemplatePolicy::Scope.new(member, EmailTemplate).resolve
